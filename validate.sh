...
case "${DOCKER_CMD[0]}" in
  *.exe)
    if command -v wslpath &>/dev/null; then
      DOCKER_BUILD_CONTEXT="$(wslpath -w "$DOCKER_CONTEXT")"
    fi
    ;;
esac

BUILD_OK=false
BUILD_OUTPUT=$(run_with_timeout "$DOCKER_BUILD_TIMEOUT" "${DOCKER_CMD[@]}" build "$DOCKER_BUILD_CONTEXT" 2>&1) && BUILD_OK=true
...
if command -v python.exe &>/dev/null; then
  OPENENV_CMD=(python.exe -m openenv.cli.__main__)
elif command -v py.exe &>/dev/null; then
  OPENENV_CMD=(py.exe -3 -m openenv.cli.__main__)
elif command -v openenv.exe &>/dev/null; then
  OPENENV_CMD=(openenv.exe)
elif command -v openenv &>/dev/null; then
  OPENENV_CMD=(openenv)
elif command -v python &>/dev/null; then
  OPENENV_CMD=(python -m openenv.cli.__main__)
elif command -v py &>/dev/null; then
  OPENENV_CMD=(py -3 -m openenv.cli.__main__)
else
